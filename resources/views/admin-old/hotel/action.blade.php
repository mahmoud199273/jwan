{!! Form::open(['method' => 'DELETE', 'route' => ["$route.destroy",$row->id], 'class' => 'form-horizontal']) !!}
        ...
        <button class="btn btn-outline-danger m-btn m-btn--icon m-btn--icon-only m-btn--custom m-btn--pill m-btn--air" title="@lang('lang.Delete')" type="submit" title="@lang('lang.Delete')" onclick="return confirm('@lang('lang.confirm Delete operation?')');"> <i class="icon-trash box-title"></i> </button>
{!! Form::close() !!}
