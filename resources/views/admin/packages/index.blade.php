@extends('admin.layouts.index_layout' , ['title' => __('admin.packages') ,'route' => 'packages'])

@section('content')

<div class="row">
    <form class="m-form" action="{{ config('app.admin_url') }}/package/search" method="get" id="search_form">
        ...
        <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-wrapper">
                    ...
                    <div class="m-portlet__head-tools">
                        ...
                        <a href="{{ config('app.admin_url') }}/packages/create" class="btn btn-brand m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                            <span>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $package->price }}</td>
                                    <td class="text-center">{!! ($package->is_active)? "<i class='fa fa-check m--font-success'></i>" : 
                                    "<i class='fa fa-times m--font-danger'></i>" !!}</td>
                                    <td>
                                        <div class="btn-group mr-2" role="group" aria-label="First group">
                                            <a type="button" href="{{ config('app.admin_url') }}/packages/{{ $package->id }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                                <i class="fa fa-eye m--font-primary"></i>
                                            </a>
                                            @if($package->is_active)
                                            <a type="button" data-id = "{{ $package->id }}"
                                            ...
                                            @endif
                                            <a type="button" 
                                            href="{{ config('app.admin_url') }}/packages/{{ $package->id }}/edit" 
            ...
            <div> 
                <span>
                    ...
                    {{$packages->currentpage()*$packages->perpage()}}
                </span> 
                <span>{{ __('admin.from') }}</span>
                <span class="badge badge-info">{{ $packages->total() }}</span>
...
<div>
    <a href="{{ config('app.admin_url') }}/packages" class="btn btn-danger m-btn m-btn--icon m-btn--wide">
        <span>
