@extends('admin.layouts.index_layout' , ['title' => __('admin.subscriptions') ,'route' => 'subscriptions'])

@section('content')

<div class="row">
    <form class="m-form" action="{{ config('app.admin_url') }}/subscription/search" method="get" id="search_form">
        ...
        <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
                    ...
                    <div class="m-section__content">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th><b>المستخدم</b></th>
                                    <th><b>اسم  صاحب الحوالة</b></th>
                                    <th><b>رقم الحوالة</b></th>
                                    <th><b>تاريخ التحويل</b></th>
                                    <th><b>حاله الموافقة</b></th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">
                                        <a href="{{ config('app.admin_url') }}/users/{{ $subscription->user->id }}" target="_blank">
                                        {{ ($subscription->user->name) ?? $subscription->user->phone  }}</a></th>

                                     <th scope="row">{{ $subscription->user_name }}</th>
                                     <th scope="row">{{ $subscription->transaction_number }}</th>
                                     <th scope="row">{{ $subscription->transaction_date }}</th>

                                    <td class="text-center">{!! ($subscription->is_approved)? "<i class='fa fa-check m--font-success'></i>" : 
                                    "<i class='fa fa-times m--font-danger'></i>" !!}</td>
                                    <td>
                                        <div class="btn-group mr-2" role="group" aria-label="First group">
                                            <a type="button" href="{{ config('app.admin_url') }}/subscriptions/{{ $subscription->id }}" class="m-btn m-btn m-btn--square btn btn-secondary">
            ...
            <div> 
                <span>
                    ...
                    {{$subscriptions->currentpage()*$subscriptions->perpage()}}
                </span> 
                <span>{{ __('admin.from') }}</span>
                <span class="badge badge-info">{{ $subscriptions->total() }}</span>
...
<div>
    <a href="{{ config('app.admin_url') }}/subscriptions" class="btn btn-danger m-btn m-btn--icon m-btn--wide">
        <span>
