@extends('admin.layouts.index_layout' , ['title' => __('admin.offices') ,'route' => 'offices'])

@section('content')

<div class="row">
    <form class="m-form" action="{{ config('app.admin_url') }}/office/search" method="get" id="search_form">
            ...
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-wrapper">
                    ...
                    <div class="m-portlet__head-tools">
                        ...
                        <a href="{{ config('app.admin_url') }}/offices/create" class="btn btn-brand m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                            <span>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $office->phone }}</td>
                                <td>
                                    @if($office->is_approved)
                                        <a type="button" data-id = "{{ $office->id }}"
                                            class="m-btn m-btn m-btn--square btn btn-secondary _reject">
                                            <i class="fa fa-times m--font-danger"></i>
                                        </a>
                                        تمت الموافقة
                                        @else
                                        <a type="button"  data-id = "{{ $office->id }}" 
                                            class="m-btn m-btn m-btn--square btn btn-secondary _approve">
                                            <i class="fa fa-check m--font-success"></i>
                                        </a>
                                        لم يتم الموافقه
                                        @endif
                                </td>
                                <td class="text-center" >{!! ($office->is_active)? "<i class='fa fa-check m--font-success'></i>" : 
                                "<i class='fa fa-times m--font-danger'></i>" !!}</td>
                                <td>
                                    <div class="btn-group mr-2" role="group" aria-label="First group">
                                        <a type="button" href="{{ config('app.admin_url') }}/offices/{{ $office->id }}" class="m-btn m-btn m-btn--square btn btn-secondary">
                                        ...
                                        @endif
                                        <a type="button" 
                                        href="{{ config('app.admin_url') }}/offices/{{ $office->id }}/edit" 
        ...
        <!--end::Section-->
            ...
            <div> 
                <span>
                    ...
                    {{($offices->currentpage()-1)*$offices->perpage()+1}} 
                    <span>{{ __('admin.to') }}</span> 
                    {{$offices->currentpage()*$offices->perpage()}}
                </span> 
                <span>{{ __('admin.from') }}</span>
                <span class="badge badge-info">{{ $offices->total() }}</span>
...
<div>
    <a href="{{ config('app.admin_url') }}/offices" class="btn btn-danger m-btn m-btn--icon m-btn--wide">
        <span>
