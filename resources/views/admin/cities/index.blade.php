@extends('admin.layouts.index_layout' , ['title' => __('admin.cities') ,'route' => 'cities'])

@section('content')

<div class="row">
    <form class="m-form" action="{{ config('app.admin_url') }}/city/search" method="get" id="search_form">
        ...
        <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group mr-2" role="group" aria-label="First group">
                                            <a type="button" 
                                            href="{{ config('app.admin_url') }}/cities/{{ $city->id }}" 
                                            class="m-btn m-btn m-btn--square btn btn-secondary">
                                            <i class="fa fa-eye m--font-primary"></i>
                                        </a>
                                        <div class="btn-group mr-2" role="group" aria-label="First group">
                                            <a type="button" 
                                            href="{{ config('app.admin_url') }}/cities/{{ $city->id }}/edit" 
                                            class="m-btn m-btn m-btn--square btn btn-secondary">
                                            <i class="fa fa-edit m--font-info"></i>
                                        </a>

                                        <a type="button"  data-id = "{{ $city->id }}" 
            ...
            <div> 
                <span>
                    ...
                    {{($cities->currentpage()-1)*$cities->perpage()+1}} 
                    <span>{{ __('admin.to') }}</span> 
                    {{$cities->currentpage()*$cities->perpage()}}
                </span> 
                <span>{{ __('admin.from') }}</span>
                <span class="badge badge-info">{{ $cities->total() }}</span>
...
<div>
    <a href="{{ config('app.admin_url') }}/cities" class="btn btn-danger m-btn m-btn--icon m-btn--wide">
        <span>
